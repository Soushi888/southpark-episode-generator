...
export const WCO_BASE = "https://www.wco.tv";

export const toWcoUrl = (watchUrl: string): string =>
  watchUrl.replace(WATCH_BASE, WCO_BASE);
export const MAX_SEASON = 28;

export const toWatchUrl = (
  ep: Pick<Episode, "season" | "episode" | "title">,
): string => {
  const slug = ep.title
    .toLowerCase()
    .replace(/[^a-z0-9\s-]/g, "")
